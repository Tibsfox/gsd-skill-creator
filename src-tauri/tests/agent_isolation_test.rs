...
async fn test_create_exec_agent() {
    ...
    assert!(
        ...
        "worktree dir must exist on filesystem"
    );
}
...
async fn test_worktree_has_planning_subtree() {
    ...
    assert!(
        planning.join("inbox").is_dir(),
        ".planning/inbox/ must exist"
    );
    assert!(
        planning.join("outbox").is_dir(),
        ".planning/outbox/ must exist"
    );
    assert!(
        planning.join("observations").is_dir(),
        ".planning/observations/ must exist"
    );
}
...
async fn test_shared_dir_created() {
    ...
    assert!(
        shared.join("integration").is_dir(),
        ".agents/shared/integration/ must exist"
    );
    assert!(
        shared.join("results").is_dir(),
        ".agents/shared/results/ must exist"
    );
}
...
async fn test_verify_isolation_false_if_agent_unknown() {
    ...
    assert!(
        !result,
        "verify_isolation must return false for unknown agent"
    );
}
