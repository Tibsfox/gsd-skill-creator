...
use crate::api::retry::{RetryOutcome, RetryPolicy};
...
fn only_retryable_errors_trigger_retry() {
    ...
    let server_500 = ApiError::HttpError {
        status: 500,
        message: "Internal".to_string(),
    };
    assert!(policy.should_retry(1, &server_500));

    let server_503 = ApiError::HttpError {
        status: 503,
    ...
    let auth_401 = ApiError::HttpError {
        status: 401,
