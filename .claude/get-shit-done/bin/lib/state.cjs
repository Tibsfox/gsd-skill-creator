...
function buildStateFrontmatter(bodyContent, cwd) {
  ...
  if (cwd) {
    try {
      ...
      // Only set milestoneName if it's a real value (not the generic fallback)
      milestoneName = (info.name && info.name !== 'milestone') ? info.name : null;
    } catch {}
  }
  ...
  if (cwd) {
    try {
      ...
      if (fs.existsSync(phasesDir)) {
        ...
        if (diskCompletedPhases > 0 || phaseDirs.length > 0) completedPhases = diskCompletedPhases;
        if (diskTotalPlans > 0) totalPlans = diskTotalPlans;
        if (diskTotalSummaries > 0) completedPlans = diskTotalSummaries;
      }
    } catch {}
  }
...
function syncStateFrontmatter(content, cwd) {
  ...
  const newFm = buildStateFrontmatter(body, cwd);

  // Deep merge: existing values preserved, new values overwrite
  ...
  for (const [key, value] of Object.entries(newFm)) {
    ...
    if (key === 'progress' && typeof value === 'object' && typeof merged.progress === 'object') {
      // Merge progress sub-object at field level
      merged.progress = { ...merged.progress };
      for (const [subKey, subVal] of Object.entries(value)) {
        if (subVal !== null && subVal !== undefined) {
          merged.progress[subKey] = subVal;
        }
      }
    } else {
      merged[key] = value;
  ...
  const yamlStr = reconstructFrontmatter(merged);
  return `---\n${yamlStr}\n---\n\n${body}`;
}
