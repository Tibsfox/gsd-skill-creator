...
function cmdPhaseComplete(cwd, phaseNum, raw) {
  ...
  // Generate phase-level SUMMARY.md if none exists
  let summaryGenerated = false;
  const dirName = phaseInfo.directory.split('/').pop();
  const phaseSummaryPath = path.join(phasesDir, dirName, 'SUMMARY.md');

  if (!fs.existsSync(phaseSummaryPath)) {
    // Gather accomplishments from plan SUMMARY one-liners
    const summaryAccomplishments = [];
    const summaryFilesChanged = new Set();
    let summaryTaskCount = 0;

    for (const s of phaseInfo.summaries) {
      try {
        const content = fs.readFileSync(path.join(phasesDir, dirName, s), 'utf-8');
        const fm = extractFrontmatter(content);
        if (fm['one-liner']) summaryAccomplishments.push(fm['one-liner']);
        // Count tasks from summary content (## Task N headings)
        const taskMatches = content.match(/##\s*Task\s*\d+/gi) || [];
        summaryTaskCount += taskMatches.length;
      } catch {}
    }

    // Extract files_modified from ALL plan frontmatter (comprehensive)
    for (const p of phaseInfo.plans) {
      try {
        const planContent = fs.readFileSync(path.join(phasesDir, dirName, p), 'utf-8');
        const planFm = extractFrontmatter(planContent);
        const fmFiles = planFm['files_modified'] || planFm['files-modified'];
        if (fmFiles) {
          const arr = Array.isArray(fmFiles) ? fmFiles : [fmFiles];
          arr.forEach(f => summaryFilesChanged.add(f));
        }
      } catch {}
    }

    // Build SUMMARY.md content
    const phaseName = (phaseInfo.phase_name || 'unnamed').replace(/-/g, ' ');
    const summaryLines = [
      `# Phase ${normalized}: ${phaseName} -- Summary`,
      '',
      `**Status:** Complete`,
      `**Date:** ${today}`,
    ...
    if (summaryAccomplishments.length > 0) {
      summaryAccomplishments.forEach(a => summaryLines.push(`- ${a}`));
    } else {
      summaryLines.push('- Phase completed (no plan one-liners recorded)');
    }

    summaryLines.push('', '## Files Changed', '');
    const changedFiles = [...summaryFilesChanged].sort();
    if (changedFiles.length > 0) {
      changedFiles.forEach(f => summaryLines.push(`- ${f}`));
    } else {
      summaryLines.push('- (no files recorded in plan frontmatter)');
    }

    summaryLines.push('', '## Test Results', '');
    summaryLines.push(`- ${summaryTaskCount} tasks executed across ${summaryCount} plans`);
    summaryLines.push('');

    fs.writeFileSync(phaseSummaryPath, summaryLines.join('\n'), 'utf-8');
