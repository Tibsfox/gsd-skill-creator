...
| Latent `.college/` → `src/` cross-import (failure mode #1) | 1 (`.college/departments/cloud-systems/extensions/runbook-interface.ts` type-imports `src/types/openstack.js`; dead code, no importers) | **CLOSED v1.49.930** — two-layer closure (#10436): source-eliminator (deleted the dead file, CF1a) + detector gate (`tools/college-src-boundary-audit.mjs` + its live Case-6 drift-guard in the gate+CI-enforced tools suite, CF1b). tsc never sees `.college/` (#10435), so the gate is the only structural enforcement of this boundary |
