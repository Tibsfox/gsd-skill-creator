...
Generated: 2026-06-05T00:46:58.495Z
Root: `.`

**Summary:** 153 modules — 98 living · 45 test-only · 10 isolated. **Reachability:** 81 reachable-from-production · 72 unreachable-from-production.
...
| `upstream` | 14 | Legacy substrate. v786 retrospective flagged that upstream + upstream-intelligence are both isolated and may represent a rename-in-flight. Pending operator triage — exempt from shelfware threshold while investigation is open. |
| `upstream-intelligence` | 7 | Legacy substrate. Companion to `upstream` — see that entry. Pending operator triage. |
...
|--------|-------------:|:-----------:|------|
| `amiga` | 1 | NO | **not allowlisted — shelfware review** |
| `audio-engineering` | 1 | NO | **not allowlisted — shelfware review** |
| `bayes-ab` | 1 | NO | **not allowlisted — shelfware review** |
| `cache` | 1 | NO | **not allowlisted — shelfware review** |
| `commands` | 1 | NO | **not allowlisted — shelfware review** |
| `components` | 1 | NO | **not allowlisted — shelfware review** |
| `dependency-auditor` | 4 | NO | **not allowlisted — shelfware review** |
| `engines` | 1 | NO | **not allowlisted — shelfware review** |
| `git` | 1 | NO | **not allowlisted — shelfware review** |
| `health-diagnostician` | 1 | NO | **not allowlisted — shelfware review** |
| `learn` | 2 | NO | **not allowlisted — shelfware review** |
| `lyapunov` | 2 | yes | allowlisted (park/reference) |
| `projection` | 1 | yes | allowlisted (park/reference) |
| `scan-arxiv` | 1 | NO | **not allowlisted — shelfware review** |
| `skill` | 1 | NO | **not allowlisted — shelfware review** |
| `skill-isotropy` | 1 | NO | **not allowlisted — shelfware review** |
| `skill-promotion` | 1 | NO | **not allowlisted — shelfware review** |
| `umwelt` | 1 | NO | **not allowlisted — shelfware review** |
...
| `predictive-skill-loader` | 12 | 3 | 11 | `chipset`, `orchestration` |
...
| `observation` | 38 | 5 | 21 | `dashboard`, `hooks`, `orchestrator` |
...
| `bounded-learning` | 27 | 7 | 18 | `chipset`, `observation`, `orchestration`, `token-budget` |
| `capabilities` | 26 | 7 | 14 | `index.ts`, `orchestrator`, `staging` |
| `integration` | 60 | 7 | 43 | `dashboard`, `hooks`, `launcher`, `terminal` |
...
| `cli` | 134 | 15 | 52 | `commands`, `scribe`, `skill` |
| `intelligence` | 207 | 26 | 121 | — |
| `validation` | 37 | 26 | 31 | `agents`, `capabilities`, `critique`, `dashboard`, `detection` (+10) |
| `security` | 10 | 39 | 45 | `agc`, `alternative-discoverer`, `aminet`, `atlas`, `cartridge` (+30) |
