...
**Test delta:** ~23,645 → **23,807 passing** (+162)
**New deps:** 0
...
Wave 3 verifies the full surface and ships this release.
...
- Post-milestone: **23,807 passing**, 2 skipped, 5 todo (23,818 total)
- Net: **+162 new tests** across Wave 0 (36), Wave 1 (37), Wave 2A (memory), 2B (12), 2C (merge assertions), 2D (19)
...
4. **Hook test location.** Vitest excludes `.claude/**` by default, so the
...
5. **PostCompact recovery combines snapshot + live state.** The recovery
...
3 Wave 2B safety-critical regressions (SC-07, SC-08 + ambiguous dashboard
flake that passed in isolation). The fixture drift and safety regressions
were repaired in two separate follow-up commits:
...
Twelve platform improvements shipped, zero new dependencies, net +162 tests.
...
| `c26ef0010` | safety-critical token restoration for SC-07/SC-08 | 3 |
