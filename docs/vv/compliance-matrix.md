...
| FRR | Flight Readiness Review | Production Go/No-Go | **Tailored** | Final system health check via post-deploy gates (`configs/evaluation/post-deploy-gates.yaml`). All service-specific health checks (keystone auth, nova compute, neutron network) must pass. Doc-verifier confirms documentation currency (SC-018). | `configs/evaluation/post-deploy-gates.yaml`; SC-018 (doc-verified accuracy); CF-CH-004, CF-CH-005 (evaluation gate tests). **Tailoring:** Formal board replaced by automated gate evaluation per NPR 7123.1 §2.2 (Type C-D, lab-scale). |
...
| Pre-deploy evaluation gates | `configs/evaluation/pre-deploy-gates.yaml` | SE-05, CDR, CF-CH-004 |
| Post-deploy evaluation gates | `configs/evaluation/post-deploy-gates.yaml` | SE-06, SIR, FRR, CF-CH-005 |
