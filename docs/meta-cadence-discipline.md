...
the project's three operational axes are in balance.
...
When all three axes are in phase, the substrate-builder / substrate-user
/ substrate-tuner ratio matches the project's stated identity as an
adaptive learning layer. When one axis lags, the project drifts toward
either pure substrate engineering (consume + calibrate lagging) or pure
post-hoc framing (codify-only).

---

## When to invoke the meta-cadence check

Before opening a counter-cadence cleanup-mission, check the three axes:
...
- Same for `--axis consume` and `--axis calibrate`.

The CLI would read manifest entries (codify), observation-source
registry wired flags (consume), and audit-log entries (calibrate)
