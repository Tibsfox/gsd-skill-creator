...
**All 14 Tier-1 items are done** (INT-1, AC-1, LEARN-3, ORCH-1/2/3/N1, CC-2, CLI-1/2/3, B2, B9, QUAL-1a) plus four Tier-2 (PG-2, PG-4, B10, **MEM-1**).

**Memory/RAG/Postgres decision — SETTLED as "wire, not quarantine"** (the operator's explicit ask was to *use* the capability). The foundation is laid and the capability is now REACHABLE: PG-2 makes the store reach the real DB, PG-4 fixes the index, and **MEM-1 (done, `f2258ea8c`)** lights up the eight `memory.*` tools end-to-end via the new `skill-creator gateway` command. The remaining capability unlock — **PG-1/PG-3** (LOD-400 tier + migration versioning) and **RET-1/RET-4** (dense recall + hybrid RRF fusion) — is the sequenced tail of §4.1, each independently shippable and default-OFF.

**Deferred (tracked here, not started):** the QUAL-1b settings-reader cascade (50-file mechanical migration — do test-first per §5, watch override-caller semantics), the PG-1/PG-3 + RET-1/RET-4 memory/RAG tail (§4.1), INT-2 (installer consolidation), ORCH-4 (atomic WorkState write), B3 (install detector), LEARN-1/2, and the rest of the Tier-2/Tier-3 backlog below. Follow-ups surfaced while wiring MEM-1 (all confirmed by adversarial review, none a live defect): `--port=` empty-value arg hygiene in the gateway command; wire a real `ConversationStore` so `memory.search_conversations` returns data instead of a graceful "not configured"; and the `MemoryServiceConfig.indexPath` naming/contract ambiguity (it is a filename joined under `memoryDir`, not an absolute path). Nothing in the deferred set is a live defect that harms users today.
