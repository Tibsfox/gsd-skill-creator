...
**Validation.** Single-instance promotion at v1.49.923 (operator-authorized codification at v1.49.924); the empirical GitHub-Actions masking fact — verified on a real throwaway-branch run — is the load-bearing evidence rather than a three-instance pattern count. The drift-guard `ci-matrix-parity.test.ts` (9 tests) is green. The load-bearing flip remains the open carry-forward, and the gate is now operationalized: `macos-flip-readiness.mjs` reports **NOT READY — streak 1/3** (only the v923 staging commit is organic-churn; all subsequent greens are release/docs ships). The checker authoring itself surfaced a predicate bug against ground truth (package*.json inflating release commits to "organic" → spurious READY 3/3), a fresh instance of #10427 (verify against ground truth, not reasoning).
