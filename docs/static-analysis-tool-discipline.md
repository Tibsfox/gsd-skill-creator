...
**Sibling gate (cargo lane) — same gate, DIFFERENT counting model (v1.49.938).** The staged `cargo` lane (a separate `continue-on-error: true` job, introduced v1.49.936 / CF4a) has its own readiness checker, `tools/ci/cargo-flip-readiness.mjs` + `tools/ci/__tests__/cargo-flip-readiness.test.mjs`. It mirrors the macOS gate's *shape* (job-conclusion read, advisory exit `0`/`1`/`2`, `detectFlipState` lifecycle awareness, defer-bias) but inverts the **counting model**, because the two legs de-risk different failure modes. The macOS leg's pre-flip risk is *cross-platform behavioral divergence on new code*, so only test-surface churn is fresh evidence and a docs/release commit re-running the identical TS surface is **inert**. The cargo lane's pre-flip risk is *lane/environment reliability*: it does a full fresh apt-install + `cargo` recompile + test on **every** push regardless of what changed, and the danger of flipping it is a transient infra flake (apt mirror, toolchain drift) red-blocking an unrelated TS ship. So for cargo a docs-only commit's green **DOES** count (it is a fresh recompile = real lane-reliability evidence) — the opposite of the macOS inert rule. A commit is *untracked* (transparent) only when it **modified the cargo lane definition** (touched `.github/workflows/ci.yml`) — its green is a self-test of that change, not independent track record (this correctly excludes the v936 introduction `ci(cargo)` commit and the future flip commit). Same misclassification bias: when in doubt (empty/missing change list, ci.yml touched, no verdict), untracked → never advance. The gate reached **READY — streak 3/3** at v1.49.938, and **v1.49.939 EXECUTED the flip**: the cargo job's `continue-on-error: true` was deleted and the STAGED assertion in `ci-matrix-parity.test.ts` was inverted to pin the line's ABSENCE (the #10461 drift-guard pairing — mutation-proven: re-adding the line reds the inverted assertion). The cargo lane is now LOAD-BEARING — its failure folds into the run-level conclusion the ship gate reads, blocking a ship exactly like an ubuntu failure. Cargo's promotion was a **two-rung** sequence (staged v1.49.936 → load-bearing v1.49.939; no decoupled-nightly first rung, unlike the macOS v920→v923→v928 three-rung arc), the gate's deterministic READY verdict driving the flip with no operator vigilance (gate-not-vigilance validated for a second lane).
