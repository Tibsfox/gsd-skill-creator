...
```bash
INIT=$(node "/media/foxy/ai/GSD/dev-tools/artemis-ii/.claude/get-shit-done/bin/gsd-tools.cjs" init phase-op "${PHASE}")
...
```bash
node "/media/foxy/ai/GSD/dev-tools/artemis-ii/.claude/get-shit-done/bin/gsd-tools.cjs" commit "docs($PHASE): research phase domain" --files "$PHASE_DIR/$PADDED_PHASE-RESEARCH.md"
```
