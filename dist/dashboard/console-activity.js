 ...
 * @returns HTML string
 */
export function renderConsoleActivity(entries, now = Date.now()) {
    if (entries.length === 0) {
    ...
    const items = limited.map((entry) => {
        ...
        const detailsHtml = entry.type === 'error' && entry.details
 ...
 * @returns CSS string
 */
export function renderConsoleActivityStyles() {
    return `
/* -----------------------------------------------------------------------
 ...
 */
export function renderClipboardFallbackScript() {
    return `<script>
(function() {
