...
// ---------------------------------------------------------------------------
function renderBinaryInputs(question) {
...
function renderChoiceInputs(question) {
    ...
    const labels = options
        .map((opt) => `<label><input type="radio" name="${question.question_id}" value="${opt}"> ${opt}</label>`)
        .join('\n      ');
    return `<div class="question-card-options">
      ${labels}
    </div>`;
}
function renderMultiSelectInputs(question) {
    const options = question.options ?? [];
    const labels = options
        .map((opt) => `<label><input type="checkbox" name="${question.question_id}" value="${opt}"> ${opt}</label>`)
        .join('\n      ');
    return `<div class="question-card-options">
      ${labels}
    </div>`;
...
function renderTextInput(question) {
    const defaultVal = typeof question.default_value === 'string' ? question.default_value : '';
    return `<textarea class="question-card-textarea" data-question-id="${question.question_id}">${defaultVal}</textarea>`;
}
function renderConfirmationInput(question) {
 ...
 * @returns CSS string.
 */
export function renderQuestionCardStyles() {
    return `
/* -----------------------------------------------------------------------
   Question Card
   ----------------------------------------------------------------------- */
