...
function renderSelect(path, label, options, current) {
    const hot = isHot(path);
    const disabledAttr = hot ? '' : ' disabled';
    const titleAttr = hot ? '' : ' title="Requires session restart to take effect"';
    const disabledClass = hot ? '' : ' setting-disabled';
...
function renderNumber(path, label, value, min, max) {
    const hot = isHot(path);
    const disabledAttr = hot ? '' : ' disabled';
    const titleAttr = hot ? '' : ' title="Requires session restart to take effect"';
    const disabledClass = hot ? '' : ' setting-disabled';
...
function renderGroup(title, controls) {
 ...
 */
export function renderConsoleSettings(config) {
 ...
 * @returns CSS string
 */
export function renderConsoleSettingsStyles() {
    return `
/* -----------------------------------------------------------------------
 ...
 */
export function renderSettingsScript(helperUrl) {
    return `<script>
(function() {
