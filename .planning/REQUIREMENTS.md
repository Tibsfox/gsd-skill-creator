...
- [ ] **TPLN-01**: Test plan generator creates categorized test specifications (safety-critical, core functionality, integration, edge cases) from vision success criteria
...
- [ ] **TPLN-03**: Safety-critical classifier identifies tests that must block deployment and marks them mandatory-pass
...
- [ ] **TPLN-05**: Test ID generator produces unique, categorized IDs: S-NNN (safety), C-NNN (core), I-NNN (integration), E-NNN (edge case)

### Template System (TMPL)

- [ ] **TMPL-01**: Template loader reads and parses all 7 vision-to-mission templates (vision, milestone-spec, component-spec, wave-plan, test-plan, readme, research-reference)
- [ ] **TMPL-02**: Template renderer substitutes placeholder tokens in templates with provided values, preserving markdown structure
...
- [ ] **TMPL-04**: Template registry provides lookup by template name with metadata (purpose, required variables, output format)
...
- [ ] **INTG-01**: Barrel exports expose complete public API from src/vtm/index.ts
- [ ] **INTG-02**: Chipset YAML for the vtm module defines skills, agents (pipeline topology), and evaluation gates
- [ ] **INTG-03**: All functions follow functional API primary + class wrapper pattern consistent with existing modules (MNA, logic sim, etc.)
