...
- [ ] **SESS-01**: Session lifecycle state machine: created → active → paused → completed/abandoned
...
- [ ] **SESS-04**: Append-only idea/question JSONL streams with session persistence
- [ ] **SESS-05**: Timer system with technique-specific defaults, pause/resume, and `TechniqueTransition` type with mandatory timer reset
