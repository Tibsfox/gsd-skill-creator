...
function getGsdStatus(startDir) {
  try {
    ...
    // Parse milestone version from frontmatter "milestone: vX.Y"
    let version = '';
    ...
    // Parse milestone name from frontmatter "milestone_name: — Some Name"
    let milestoneName = '';
    ...
    if (!phaseNum && !status && !progress && !version) return null;

    // --- Build colorful GSD status string ---
    // Each sub-part gets its own color for visual scanning
    ...
    // Milestone name — from frontmatter (preferred) or phase name fallback
    // More generous truncation (40 chars) so it actually shows useful info
