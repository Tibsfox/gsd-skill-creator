...
log "[pre-tag-gate] step 1/14: PASS"
...
log "[pre-tag-gate] step 1.5/14: version-sequence sanity"
...
log "[pre-tag-gate] step 1.5/14: PASS"

log "[pre-tag-gate] step 2/14: npx vitest run (full suite — mirrors CI)"
...
log "[pre-tag-gate] step 2/14: PASS"

log "[pre-tag-gate] step 3/14: release-notes completeness gate"
...
log "[pre-tag-gate] step 3/14: PASS"

# ----- step 4/14: CI-on-dev gate (v1.49.587 HARD RULE) -----
...
if gate_bypassed "ci-gate" "SC_SKIP_CI_GATE"; then
  log "[pre-tag-gate] step 4/14: SKIPPED"
else
  log "[pre-tag-gate] step 4/14: CI-on-dev verification"
  ...
  if [ "$CONCLUSION" != "success" ]; then
    ...
    if [ -n "${SC_SKIP_CI_GATE_TESTS:-}" ]; then
      ...
      if node scripts/ci-gate-enum.mjs; then
        log "[pre-tag-gate] step 4/14: PASS (CI red(s) authorized via SC_SKIP_CI_GATE_TESTS at $DEV_SHA)"
    ...
    log "[pre-tag-gate] step 4/14: PASS (CI green at $DEV_SHA)"
  fi
fi

# ----- step 5/14: SPICE renderer bundle freshness (v1.49.587 unwired-build closure) -----
log "[pre-tag-gate] step 5/14: www-bundles freshness (SPICE renderer)"
...
log "[pre-tag-gate] step 5/14: PASS"

# ----- step 6/14: depth-audit (BLOCKER as of v1.49.591; closes Lesson #10188) -----
...
if gate_bypassed "depth-audit" "SC_SKIP_DEPTH_AUDIT"; then
  log "[pre-tag-gate] step 6/14: SKIPPED"
else
  log "[pre-tag-gate] step 6/14: depth-audit (BLOCKER mode — hardened at v1.49.591; cross-link strict at v1.49.595+)"
  if [ -f "$REPO_ROOT/.planning/STATE.md" ]; then
    ...
    if gate_bypassed "depth-audit-mus-elc" "SC_SKIP_DEPTH_AUDIT_MUS_ELC"; then
      ...
      log "[pre-tag-gate] step 6/14:   MUS/ELC findings downgraded (SC_SKIP_DEPTH_AUDIT_MUS_ELC=1)"
    fi
    ...
    log "[pre-tag-gate] step 6/14: PASS (depth-audit clean)"
  else
    log "[pre-tag-gate] step 6/14: SKIPPED (.planning/STATE.md absent — cannot derive version)"
  fi
fi

# ----- step 7/14: CLAUDE.md auto-render drift check -----
if gate_bypassed "claude-md" "SC_SKIP_CLAUDE_MD_GATE"; then
  log "[pre-tag-gate] step 7/14: SKIPPED"
else
  log "[pre-tag-gate] step 7/14: CLAUDE.md auto-render drift check"
...
if gate_bypassed "catalog-index" "SC_SKIP_CATALOG_INDEX_GATE"; then
  ...
  log "[pre-tag-gate] step 8/14: catalog-index drift check (BLOCKER mode — added v1.49.601)"
  ...
  log "[pre-tag-gate] step 8/14: PASS"
fi

# ----- step 9/14: tauri-boundary audit (added v1.49.634 §15) -----
if gate_bypassed "tauri-boundary" "SC_SKIP_TAURI_BOUNDARY_GATE"; then
  log "[pre-tag-gate] step 9/14: SKIPPED"
else
  log "[pre-tag-gate] step 9/14: tauri-boundary audit"
  ...
  log "[pre-tag-gate] step 9/14: PASS"
fi
...
if gate_bypassed "apply-to-self" "SC_SKIP_APPLY_TO_SELF"; then
  log "[pre-tag-gate] step 9.5/14: SKIPPED"
else
  log "[pre-tag-gate] step 9.5/14: apply-to-self enforcement"
  ...
  log "[pre-tag-gate] step 9.5/14: PASS"
fi

# ----- step 10/14: scaffolder-residue audit (v1.49.653, CONCERNS §18.2) -----
...
if gate_bypassed "scaffolder-residue" "SC_SKIP_SCAFFOLDER_RESIDUE_GATE"; then
  log "[pre-tag-gate] step 10/14: SKIPPED"
else
  log "[pre-tag-gate] step 10/14: scaffolder-residue audit"
  ...
  log "[pre-tag-gate] step 10/14: PASS"
fi

# ----- step 11/14: citation-debt ledger sync (v1.49.653, CONCERNS §9.3) -----
...
if gate_bypassed "citation-debt-sync" "SC_SKIP_CITATION_DEBT_GATE"; then
  log "[pre-tag-gate] step 11/14: SKIPPED"
else
  log "[pre-tag-gate] step 11/14: citation-debt ledger sync scan"
  ...
  if [ "$CITATION_SCAN_EXIT" -ne 0 ]; then
    ...
    log "[pre-tag-gate] step 11/14: WARN (informational; set SC_PRE_TAG_GATE_REQUIRE=citation-debt-sync to block)"
  else
    log "[pre-tag-gate] step 11/14: PASS (no V-flag activity)"
  fi
fi
...
if gate_bypassed "story-drift" "SC_SKIP_STORY_DRIFT_GATE"; then
  ...
  log "[pre-tag-gate] step 12/14: STORY.md drift detection"
...
# ----- step 13/14: discipline coverage audit (v1.49.653, L-04, CONCERNS §23.4) -----
...
if gate_bypassed "discipline-coverage"; then
  log "[pre-tag-gate] step 13/14: SKIPPED"
else
  log "[pre-tag-gate] step 13/14: discipline coverage audit"
  ...
  if [ "${UNCODIFIED_COUNT:-0}" -gt 0 ]; then
    ...
    log "[pre-tag-gate] step 13/14: WARN (informational; set SC_PRE_TAG_GATE_REQUIRE=discipline-coverage to block)"
  else
    log "[pre-tag-gate] step 13/14: PASS (no uncodified lessons)"
  fi
fi

# ----- step 14/14: SPS cohort-uniqueness audit (v1.49.666, Lesson #10364 codification) -----
...
if gate_bypassed "sps-cohort-uniqueness" "SC_SKIP_SPS_COHORT_UNIQUENESS_GATE"; then
  log "[pre-tag-gate] step 14/14: SKIPPED"
else
  log "[pre-tag-gate] step 14/14: SPS cohort-uniqueness audit"
  ...
  if echo "$SPS_UNIQ_OUTPUT" | grep -qE "COLLISION-DETECTED|PROSPECTIVE.*COLLISION"; then
    ...
    log "[pre-tag-gate] step 14/14: WARN (informational; set SC_PRE_TAG_GATE_REQUIRE=sps-cohort-uniqueness to block)"
  else
    log "[pre-tag-gate] step 14/14: PASS (no SPS-number collisions)"
  fi
fi

log "[pre-tag-gate] all 14 checks PASS — safe to \`git tag\` and merge to main"
exit 0
