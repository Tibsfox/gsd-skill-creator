#!/usr/bin/env node
/**
 * tools/ci/windows-flip-readiness.mjs — operationalizes the #10463 flip gate for
 * the STAGED windows matrix leg (introduced v1.49.985, Phase 4).
 *
 * Lesson #10463 (Static-analysis tool authoring; staged CI-lane promotion) says a
 * matrix leg flips from STAGED non-blocking (`continue-on-error`) to LOAD-BEARING
 * only "once N consecutive green pushes of the new leg accumulate — and 'green
 * pushes from the promotion ship itself' do not count as a diversity track record;
 * the data must come from organic development churn across diverse changes"
 * (docs/static-analysis-tool-discipline.md). That gate is prose-only and easy to
 * misjudge (at v1.49.924 all 9 macOS greens were docs/CI ships — zero organic churn
 * — yet the run-level conclusion was uniformly `success`). This tool makes the gate
 * for the windows leg DETERMINISTIC and CHECKABLE, in the gate-not-vigilance spirit
 * of #10428 / two-layer-closure. It is a direct analog of macos-flip-readiness.mjs
 * (windows is a test-matrix leg exactly like macOS, so it uses the same ORGANIC-
 * churn model — NOT the cargo lane-stability model).
 *
 * LIFECYCLE: the windows leg is being introduced STAGED at v1.49.985; the flip to
 * LOAD-BEARING is PENDING — it executes (delete the gated `continue-on-error`,
 * paired with the ci-matrix-parity.test.ts update) once this tool reports READY N/N
 * across organic churn. The tool reads ci.yml to report the current state: while the
 * gated `continue-on-error` is present the READY guidance says "safe to flip: delete
 * the line"; once the line is gone (post-flip) it switches to "already flipped; here
 * is how to REVERT" (the streak math stays informational — a tool that kept telling
 * operators to "delete continue-on-error" after the line was already gone would be
 * the stale-guidance failure #10427 warns against).
 *
 * Readiness = the most recent run of `consecutive` distinct ORGANIC-churn commits
 * on the windows leg were ALL green, and that count >= N. INERT commits (docs /
 * release / CI-config only — they re-run an unchanged test surface) are
 * TRANSPARENT: they neither count toward the streak nor break it. A windows-leg
 * FAILURE on an organic commit BREAKS the streak (resets the track record).
 *
 * ORGANIC churn = a commit that changed any path the windows matrix leg actually
 ...
 *
 * Health is read from the JOB conclusion, never the RUN conclusion. While STAGED the
 * windows leg's `continue-on-error` masks a red windows leg OUT of the run-level
 * conclusion (#10463 empirical masking fact), so `gh run list ... .conclusion` is
 * uniformly `success` and useless here. Post-flip the windows outcome IS folded into
 * the run conclusion, but the per-leg JOB conclusion is still the cleaner signal (the
 * run conclusion aggregates ubuntu+macOS+windows and cannot attribute a red to the
 * windows leg specifically). Either way: read the JOB.
 *
 ...
 *
 * Usage:
 *   node tools/ci/windows-flip-readiness.mjs                 # live: query gh + local git
 *   node tools/ci/windows-flip-readiness.mjs --json          # machine-readable summary
 *   node tools/ci/windows-flip-readiness.mjs --n=5           # override the track-record bar
 *   node tools/ci/windows-flip-readiness.mjs --limit=50      # scan more recent runs
 *   node tools/ci/windows-flip-readiness.mjs --runs-file=F   # inject runs JSON (tests; no gh/git)
 *   node tools/ci/windows-flip-readiness.mjs --root=DIR      # git root for churn derivation
 *   node tools/ci/windows-flip-readiness.mjs --ci-file=F     # ci.yml path for flip-state detection
 *
 * --runs-file JSON: array NEWEST-FIRST of run records. Each record:
 *   { "sha": "<full-or-short>", "windowsConclusion": "success"|"failure"|...,
 ...
 *
 * Exit codes:
 *   0  READY   — >= N consecutive organic-churn green windows runs accumulated
...
// Test-bearing roots the windows matrix leg exercises (`npx vitest run`,
...
// plausibly make windows diverge from ubuntu.
...
// every chore(release) commit touches BOTH, so including them would mark every
// release commit "organic" — the exact systematic false-positive the gate exists
// to prevent (the live run at v924 reported a spurious READY 3/3 before the
// macOS tool's equivalent exclusion). The cost is a conservative MISS: a rare pure
// dependency upgrade that touches only package*.json is undercounted as inert.
// That only DEFERS the flip (the safe direction per the tight-allow-list bias),
// so we accept it over a gate that flips on release noise. A dep upgrade that also
// touches src/ is still organic via the roots.
...
// The staged windows leg carries `continue-on-error: ${{ matrix.os == 'windows-latest' }}`.
// The load-bearing flip DELETES that line. Detect which state ci.yml is in so the
// readiness guidance stays honest across the lifecycle (#10427: a tool that keeps
// saying "safe to flip: delete continue-on-error" after the line is gone is itself
// the stale-guidance failure it exists to prevent).
const STAGED_WINDOWS_COE =
  /continue-on-error:\s*\$\{\{\s*matrix\.os\s*==\s*'windows-latest'\s*\}\}/;

/**
 * Classify ci.yml content as 'staged' (windows leg still non-blocking — the gated
...
export function detectFlipState(ciYmlText) {
  ...
  return STAGED_WINDOWS_COE.test(ciYmlText) ? 'staged' : 'flipped';
}

// ─── the streak computation (pure) ───────────────────────────────────────────

// A windows-leg conclusion that counts as a clean green.
...
/**
 * @param {Array<{sha:string, windowsConclusion:string, churn:'organic'|'inert', title?:string, branch?:string}>} runs
...
export function computeReadiness(runs, { n }) {
  ...
  for (const r of runs) {
    ...
    const concl = r.windowsConclusion ?? null;
    if (churn !== 'organic') {
      inertSkipped += 1;
    ...
    if (GREEN.has(concl)) {
      ...
      broke = { sha: r.sha, windowsConclusion: concl, title: r.title };
      stopped = true;
...
/** windows-leg job conclusion for one run id (null if no windows job present). */
function fetchWindowsConclusion(runId, repo) {
  const out = gh([
    'run', 'view', String(runId), '--repo', repo, '--json', 'jobs',
    '--jq', '[.jobs[] | select(.name|test("windows")) | .conclusion] | first',
  ]);
  const v = out.trim();
...
function buildLiveRuns({ repo, limit, root, n }) {
  ...
  for (const row of rows) {
    ...
    seen.add(sha);
    const windowsConclusion = fetchWindowsConclusion(row.databaseId, repo);
    ...
    if (churn === 'organic') {
      if (GREEN.has(windowsConclusion)) organicGreens += 1;
      else if (BREAKING.has(windowsConclusion)) break; // streak broken; no need to look further
    }
...
function renderHuman(result, runs) {
  const lines = [];
  lines.push('windows matrix-leg flip readiness (#10463 staged → load-bearing gate)');
  lines.push('─'.repeat(68));
  for (const d of result.detail) {
    const mark = d.counted ? '✓' : d.reason.includes('BREAKS') ? '✗' : '·';
    const concl = d.windowsConclusion ?? 'n/a';
    lines.push(
  ...
  if (result.broke) {
    lines.push(`  streak broken by organic ${result.broke.windowsConclusion} at ${result.broke.sha}`);
  }
  const flipped = result.flipState === 'flipped';
  if (result.ready) {
    lines.push('');
    lines.push(`  VERDICT: READY ✓ — ${result.streak} consecutive organic-churn green windows runs.`);
    if (flipped) {
      lines.push('  The windows leg is ALREADY load-bearing — the gated `continue-on-error`');
      lines.push('  has been deleted from ci.yml; this readout is informational.');
    ...
    lines.push('');
    lines.push(
      `  VERDICT: NOT READY — need ${need} more consecutive organic-churn green windows push(es).`,
    );
    lines.push('  Docs/release/CI-only ships do NOT count (they re-run an unchanged test surface).');
    if (flipped) {
      lines.push('  NOTE: the windows leg is ALREADY load-bearing; this streak is');
      lines.push('  historical/informational, not a pending-flip gate.');
    }
...
function main() {
  ...
  try {
    if (args.runsFile) {
      ...
      runs = raw.map((r) => ({
        sha: r.sha,
        windowsConclusion: r.windowsConclusion ?? null,
