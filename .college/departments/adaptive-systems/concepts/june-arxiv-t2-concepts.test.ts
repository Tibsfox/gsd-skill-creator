/**
 * adaptive-systems Department -- June-2026 arXiv T2 concept tests.
 *
 ...
 * three panels, id prefix adaptive-systems-, and dept-local targetId resolution
 * (cross-dept / foundational refs accepted).
 */

import { describe, it, expect } from 'vitest';
import {
  functionalIdentifiability,
...
describe('adaptive-systems Department -- June-2026 arXiv T2 concepts', () => {
  describe('T2-01: Valid fields + domain', () => {
    it.each(t2Concepts.map((c, i) => [t2ConceptNames[i], c] as const))(
      '%s has id, name, domain=adaptive-systems, description', (_n, c) => {
  ...
  describe('T2-05: id prefix adaptive-systems-', () => {
    it.each(t2Concepts.map((c, i) => [t2ConceptNames[i], c] as const))(
      '%s id starts with adaptive-systems-', (_n, c) => {
  ...
  describe('T2-06: Dept-local targetId resolution', () => {
    ...
    it.each(t2Concepts.map((c, i) => [t2ConceptNames[i], c] as const))(
      '%s adaptive-systems- targetIds resolve', (_n, c) => {
