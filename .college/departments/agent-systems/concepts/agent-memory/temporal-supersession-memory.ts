...
export const temporalSupersessionMemory: RosettaConcept = {
  ...
  description:
    "A retrieval memory conflates two very different relations under one geometry: cosine similarity cannot separate a contradiction from a duplicate, so a vector store co-retrieves a stale fact alongside the very replacement that should have retired it (arXiv 2606.26511v1, 2026). The remedy is not a sharper embedding but a deterministic layer above it — timestamp every fact and key it by a structured (subject, relation, object) triple, so a newer fact sharing subject and relation supersedes, and retires, the prior object rather than sitting beside it in the top-k. Hash-based deduplication catches only byte-identical repeats; contradiction-retirement is exactly the semantic case it misses. The transferable lesson for agent systems: recency and non-contradiction are invariants a retriever must enforce structurally, not properties to hope the vector space happens to encode.",
  panels: new Map(),
  relationships: [
    {
