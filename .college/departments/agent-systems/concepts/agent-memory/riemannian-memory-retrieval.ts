...
export const riemannianMemoryRetrieval: RosettaConcept = {
  ...
  description:
    "Long-term agent memory usually ranks stored records by cosine similarity in one shared embedding space, but in high dimensions a few \"hub\" vectors become spurious nearest neighbors to nearly every query, silently corrupting recall as the store grows. CoreMem (arXiv:2606.18406v1) is an edge-cloud architecture built for a strict 8 GB-VRAM budget that replaces this isotropic metric with a locally adaptive Fisher-Rao (Riemannian) distance, penalizing hub memories via a Mahalanobis distance made real-time by O(Ndr) Woodbury acceleration — which is what keeps a curved metric affordable rather than intractable. Its second half, Fisher-guided discrete token distillation (FDTD), is a hierarchical sentence-to-token compression that derives sensitivity scores from Fisher-information traces, trading compression against KL divergence while explicitly protecting syntactic structure. On the LOCOMO and LongMemEval-S benchmarks the combination lifts open-domain reasoning by +4.51 pp and temporal reasoning by +4.17 pp, all within the 8 GB budget. For agents, retrieval accuracy then holds up under large, skewed memory instead of degrading in ways a flat cosine score cannot expose.",
  panels: new Map(),
  relationships: [
    {
      type: "dependency",
