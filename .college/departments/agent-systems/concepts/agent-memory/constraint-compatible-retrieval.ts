...
export const constraintCompatibleRetrieval: RosettaConcept = {
  ...
  description:
    "Dense retrieval ranks by topical similarity, so a query for \"flights with no layover\" scores layover-heavy passages as top matches — negations and excluded attributes get surfaced as evidence even though they violate the query. CoDeR (arXiv 2606.13204) uses a dual-encoder architecture that decouples two functions: a standard relevance encoder for topical coverage and a separate compatibility bi-encoder — trained with lexical-polarity supervision — that scores whether a document satisfies or violates the stated constraint, with the two combined in a rescoring step so the retriever enforces a constraint rather than treating it as a match. \"Local\" denotes constraint checking at the document level: catching when specific evidence inside a topically relevant passage contradicts the query. On controlled diagnostics it cuts violations at rank 2 (V@2) by 20.59 points for antonymy, 23.53 for negation, and 5.77 for exclusion versus baselines, all without any external LLM call at inference. For agents whose memory holds negated preferences or hard exclusions, this stops recall from returning exactly what the user ruled out.",
  panels: new Map(),
  relationships: [
    {
      type: "dependency",
