...
export const admissionTimeHubnessGate: RosettaConcept = {
  ...
  description:
    "Vector memory carries a write-side integrity gap: an attacker can craft a document whose embedding sits near many unrelated query regions, so once inserted it becomes the nearest neighbour of queries it should never match — poisoning that weaponizes hubness, the high-dimensional geometry phenomenon where a few points dominate everyone's k-nearest-neighbour lists (arXiv 2606.19692v1, 2026). The defense scores each incoming record against a bank of sentinel queries and quarantines hub-like candidates before insertion, rather than filtering at read time. The paper's sharper claim — captured in its title, \"When Global Gating Is Enough\" — is that a single GLOBAL admission gate suffices: a per-topic gate adds no reliable benefit, because anisotropy couples a point's local and global visibility, so screening once against a diverse sentinel bank is enough. The gate is cheap and precise: it adds only ~3% ingestion latency on an HNSW index while holding roughly 1% false positives on benign documents. This names the missing corner of the memory-defense taxonomy: read-side hardening guards what comes back, but only admission-time screening guards what goes in. For agents that append tool outputs, retrieved passages, and self-authored notes into shared memory, every write becomes a trust decision, not a free operation.",
  panels: new Map(),
  relationships: [
    {
      type: "dependency",
