...
export const indexSideReasoning: RosettaConcept = {
  ...
  description:
    "Standard retrieval reasons over relations at query time — expanding, chaining, or rewriting the query per request — which is slow and repeats the same relational work on every hit. RL-Index (arXiv:2606.16316) reframes index construction as a reinforcement-learning problem: an agent reasons over the corpus offline, materializing implicit cross-document relations directly into the index structure, using downstream retrieval quality as the reward signal. At serving time the query path stays cheap because the hard relational reasoning was already baked into the index rather than recomputed per query. For agent memory this decouples expensive multi-hop reasoning from latency-sensitive recall, letting a large corpus be pre-reasoned once and queried cheaply many times.",
  panels: new Map(),
  relationships: [
    {
      type: "dependency",
