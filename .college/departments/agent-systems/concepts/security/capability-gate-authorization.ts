...
export const capabilityGateAuthorization: RosettaConcept = {
  ...
  description:
    "Framework defaults conflate tool EXPOSURE with AUTHORIZATION: making a side-effecting tool available to an agent (capability gating) is not the same as authorizing a specific call with concrete argument values. When an agent reads untrusted content while holding tools for payments, email, CRM, or infrastructure, that gap is a confused-deputy vulnerability — the agent is steered into wielding authority it holds but should not use here. The ScopeGate audit (arXiv 2606.28679, 2026) finds LangChain/LangGraph, LlamaIndex, and the Stripe Agent Toolkit all provide capability gating by default but none a deterministic fail-closed per-call value-authorization gate. Its five-stage PDP/PEP — scope, authorization, money ceiling, idempotency, default-deny — re-authorizes each model-emitted call before execution, denying an unauthorized payout that executes under default dispatch. In the reported audit the gate holds at 0/48 static bypasses, 0/29 unauthorized attempts over a 40-iteration adaptive run, 0/10 benign false-denies, and 10/10 Latam-GPT payment-agent containment. Implication: authorize the CALL, not the capability; default deny.",
  panels: new Map(),
  relationships: [
    {
      type: "dependency",
