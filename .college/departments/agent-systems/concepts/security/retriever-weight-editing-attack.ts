...
export const retrieverWeightEditingAttack: RosettaConcept = {
  ...
  description:
    "CAREATTACK is a model-centric attack that injects malicious knowledge into retrieval-augmented generation by editing the retriever's parameters rather than the corpus (arXiv 2606.18310, 2026). It proceeds in two stages: conflict-aware retriever editing applies closed-form parameter edits to a dense retrieval model, promoting malicious passages above benign competitors and resolving parameter conflicts through graph-based conflict detection and an editing-projection step; then attack-preserving anchor repair lightly calibrates the edited retriever to remove side effects on non-target prompts while keeping the attack effective for targeted ones. Instantiated on Qwen3-Embedding-0.6B and BGE-M3 and evaluated across three benchmark datasets, it substantially promotes malicious passages into the retrieved set and — given white-box access to the retriever's parameters — scales to attack batches of target prompts and passages at once, a severity multiplier over single-prompt attacks. Because synthetic malicious corpora are often detectable, moving the attack into the weights of widely reused open-source retrievers exposes a stealthier surface. For agent builders, it shows that vetting a knowledge base is insufficient: the retriever checkpoint itself must be treated as untrusted and integrity-verified.",
  panels: new Map(),
  relationships: [
    {
