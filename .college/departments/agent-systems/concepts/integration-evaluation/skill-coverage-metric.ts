...
export const skillCoverageMetric: RosettaConcept = {
  ...
  description:
    "Test-adequacy for skills: translate a skill's natural-language instructions into semi-structured behavioural constraints — each an 'under condition C, expect behaviour X' clause — then measure over a trajectory which of them were exercised and, where they were, whether the agent obeyed them (arXiv 2606.20659v2, 2026). It is a crisp transposition of code coverage into the skill/trajectory domain — an axis distinct from counterfactual effect and from bare compliance. It sorts a skill's clauses into three states: exercised-and-followed, exercised-but-violated (a compliance gap), and never-exercised (untested). On the SkillsBench leaderboard, agent trajectories cover only 38.66-45.51% of the extracted constraints on average; re-emphasizing only the instructions a skill failed to follow and re-running recovers ~16.0% of the failed tasks — the closed loop that makes coverage worth computing. The failure mode it surfaces is the untested instruction: a clause that ships but no probe triggers, so its counterfactual effect is unmeasurable and its compliance unknown. The implication: coverage tells you whether an audit's silence means no effect or merely not-yet-exercised, steering probe-bank expansion at un-covered clauses.",
  panels: new Map(),
  relationships: [
    {
