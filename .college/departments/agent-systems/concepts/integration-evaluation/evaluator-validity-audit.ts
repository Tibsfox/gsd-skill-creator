...
export const evaluatorValidityAudit: RosettaConcept = {
  ...
  description:
    "Turn the auditing lens onto the evaluator itself: a tool-calling benchmark's scoring harness is a fallible artifact, not ground truth, and it fails in named, recurring ways — brittle final-state matching, trajectory lock-in that credits only one blessed path, mislabelled ground truths, and LLM-judge rubric drift with run-to-run variance. The 2026 finding (arXiv 2607.02577v1, 2026) is that meta-evaluating the evaluator surfaces scoring errors large enough to reorder agent leaderboards, so a benchmark's numbers cannot be trusted until its scorer has itself been validated. This flips the audited object from the agent trace to the measurement instrument. The implication for building agent systems: every gate, utility estimate, and keep/retire decision inherits the validity of its evaluator, so certify the scorer before acting on its scores.",
  panels: new Map(),
  relationships: [
    {
