...
export const layerIsolatedEval: RosettaConcept = {
  ...
  description:
    "Layer-isolated evaluation decomposes a deployed LLM agent into a fixed taxonomy of layers—ontology, intent, routing, decomposition, escalation, safety, memory, and a cross-cutting envelope/defense layer—and exercises each with its own deterministic, no-LLM assertion slice run in CI against a locked per-slice baseline (arXiv 2606.11686, 2026); the pure suite is 238 cases across 23 slices at ~10 ms/case, completing in about 2.39 s. Its distinct contribution is exposing masking: under controlled single-layer regression injection across seven non-safety layers, the aggregate task-success pass-rate barely moves (−1.7 to −5.9 pp) while the matching slice craters (−25 to −91 pp), and the damage stays off other slices — the injected layer's slice is the single worst-hit in 5 of 7 cases and top-3 in 7 of 7 (mean rank 1.29 of 19), a result that replicates on a second, structurally different tenant. So per-slice baseline-locked gates localize regressions that one aggregate number hides. A coverage-honesty criterion further refuses to score any layer left unexercised. For agent builders, the implication is to gate CI on fast, reproducible component-level slices rather than a single end-to-end score, turning \"the agent regressed\" into \"this layer regressed.\"",
  panels: new Map(),
  relationships: [
    {
      type: "analogy",
