...
export const declarativeAgentControl: RosettaConcept = {
  ...
  description:
    "An agent handed natural-language skill files in its system prompt directs its own control flow, rather than being stepped through an externally coded state machine. The 2026 formalization (arXiv 2606.06923v1, 2026) names this the declarative orchestration paradigm and sets it against imperative scaffolding along a single control-flow-locus axis: declarative policies vest orchestration in the agent reading NL specs, imperative policies vest it in code the agent merely obeys. This is exactly the paradigm the skill-library approach embodies — the skill file is the policy, not a subroutine. The paper formalizes three agents as policy classes in a Dec-POMDP — a DeclarativeAgent reading three domain-specific skill files, an ImperativeAgent programmatic state machine, and an unscaffolded tau-Knowledge baseline — and tests them across five language models and two retrieval regimes. Its headline finding is that retrieval quality is the dominant bottleneck: when evidence is incomplete or skewed all agents degrade substantially and skill files cannot recover the lost performance. Only under high-quality retrieval do declarative skills consistently improve procedural accuracy and reduce orchestration errors, while the imperative state machine's brittleness does not reliably improve task success or compliance. Choosing the control-flow locus is therefore an architectural decision whose payoff is gated on retrieval quality, not an implementation detail.",
  panels: new Map(),
  relationships: [
    {
