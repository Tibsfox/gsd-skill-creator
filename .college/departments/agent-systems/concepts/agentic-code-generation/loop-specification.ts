...
export const loopSpecification: RosettaConcept = {
  ...
  description:
    "Step-by-step prompting makes a human the agent's runtime scheduler, re-issuing instructions every turn. Loop Engineering (arXiv 2607.00038, 2026) replaces this with a loop specification: a bounded, reusable artifact bundling five fields — trigger, goal, a verification step, a stopping rule, and memory — handed once to an agent harness, which then pursues the goal on its own. The paper's anatomy organizes these around a goal type, a five-level verification ladder, an architecture, and named terminal states, and carefully distinguishes the loop specification from two things it is confused with: an ordinary programming loop, and the harness's own internal perceive-act-observe cycle. Its evidence is the Loop Library, a public corpus of fifty real loops hand-coded by the authors: 70% verify in the autonomous zone of the ladder and 74% name their terminal states, while automated triggering and durable memory remain comparatively underdeveloped — practice has matured where the discipline says it matters. The insight is externalizing the control loop as a declarative object separate from the harness that runs it, so autonomy becomes something you author, version, and reuse rather than improvise per turn; yet loop engineering does not retire prompt engineering — loop and prompt are distinct tools with distinct uses. It matters because it turns \"how autonomous should this agent be\" into an inspectable, testable input instead of emergent runtime behavior.",
  panels: new Map(),
  relationships: [
    {
      type: "dependency",
