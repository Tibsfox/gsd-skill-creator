...
export const nativeComputerUse: RosettaConcept = {
  ...
  description:
    "Agent-Native Computer Use rejects the dominant GUI-centric paradigm—where agents drive software by interpreting screenshots, locating UI elements, and issuing coordinate-based mouse clicks—as a fundamental misalignment with agent strengths (arXiv 2606.03854, 2026). CLI-Anything argues that forcing agents to emulate human perceptual limits produces brittle pixel-level interactions and timing dependencies that break when interfaces change. Its distinct move is to redesign the interaction surface itself: transform existing applications into command-line harnesses that preserve functionality while exposing structured commands, explicit state representations, and deterministic feedback through machine-readable protocols. This eliminates the lossy visual-to-computational translation that plagues screen-reader-and-click-simulator stacks, and the CLI-Hub platform operationalizes the methodology, architecture, and infrastructure. For building agent systems, the implication is to expose programmatic, deterministic control surfaces rather than automate human-facing GUIs.",
  panels: new Map(),
  relationships: [
    {
      type: "dependency",
