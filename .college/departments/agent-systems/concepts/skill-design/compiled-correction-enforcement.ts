...
export const compiledCorrectionEnforcement: RosettaConcept = {
  ...
  description: 'Agent Compiled Correction Enforcement is the pattern of turning a user\'s in-chat corrections into runtime PASS-gates that an agent must satisfy before it is allowed to mark a future task complete, rather than merely storing the correction as a memory the model may or may not consult. The named instance, TRACE (Test-time Rule Acquisition and Compiled Enforcement) from arXiv:2606.13174 (2026), is a drop-in skill-layer pipeline for coding-agent runtimes that mines corrections from the user\'s own chat, rewrites each into an atomic, self-contained rule, and compiles that rule into an executable check wired into the completion path — so a violated preference blocks task closure the way a failing test would, unlike developer-authored guardrails written ahead of time. It targets the gap the paper calls preference ACCESS versus preference COMPLIANCE: Mem0-style memory can recall a correction yet still leaves 57.5% of applicable preference checks violated. Compiled enforcement collapses that gap, cutting held-out preference violation on ClawArena from 100.0% to 37.6% in-distribution and from 100.0% to 2.0% out-of-distribution, while matching or beating the strongest memory baseline on task pass. The size of the collapse is domain-dependent, though — on the harder MemoryArena-derived tasks the same pipeline only cuts in-distribution violation from 100.0% to 60.5% (still matching or exceeding the strongest memory baseline on task pass), and the striking ClawArena in-vs-out-of-distribution asymmetry (37.6% versus 2.0%) suggests a compiled check generalizes best precisely on the novel tasks where a memory lookup is least likely to be consulted. Distinct from Trace-to-Skill Induction, which segments a trajectory into a reusable, generically applicable skill spec, this mechanism\'s unit of value is a single user correction and its output is a hard runtime gate on task completion, not a capability the agent gains. The implication for agent systems: persistence of a preference should be measured by whether the next run is blocked when it is violated, not by whether the preference can be retrieved — the enforcement path, not the memory store, is where "getting easier to work with over time" actually lives, so wire corrections into the completion gate.',
  panels: new Map(),
  relationships: [
    {
      type: 'analogy',
