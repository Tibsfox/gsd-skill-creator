...
export const traceToSkillInduction: RosettaConcept = {
  ...
  description:
    "Skills need not be hand-authored: they can be induced backwards from heterogeneous interaction evidence (demonstrations, trajectories, tool traces, execution logs) by segmenting each trace into reusable units (arXiv 2606.06893v1, 2026). The finding is that a soundly induced skill decomposes into three layers: workflow structure (the ordered scaffold), execution semantics (what each step means and mutates), and runtime attachments capturing verification, safety, rollback, and state management. The W2S pipeline that produces these entries runs five steps — segment the raw traces, induce local skill drafts, align shared structures across them, reconcile divergent branches, and compress redundancy while preserving each step's evidence and confidence annotations. This W2S/RWSA decomposition names an axis the IR manifest omits: a compilation manifest declares tools, inputs, dependencies, ordering, and output shape, but nothing about how to verify a step, roll it back, or recover mid-run. The implication: the library grows from what the agent already did rather than from what an author remembered to write, each induced entry carrying its own safety envelope. Across 70 induced skills this lifts behavioral-replay consistency by 10.5% over summarization- and prompting-based baselines — the paper's evidence that traces are executable runtime specifications, not compressible text.",
  panels: new Map(),
  relationships: [
    {
