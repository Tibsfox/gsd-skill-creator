...
export const skillInternalization: RosettaConcept = {
  ...
  description:
    "Externalized skill libraries force an agent to run a skill generator and query an inference-time skill bank on every task, adding retrieval latency and a maintenance surface. SIRI (arXiv 2606.02355) instead mines candidate skills from the agent's own successful rollouts, validates each by paired execution — running the task with and without the skill and keeping only those that measurably help — then fine-tunes the surviving behavior back into the policy weights. Skills become parametric rather than artifacts, so no external generator or runtime skill bank is needed; the cost is that internalized skills are no longer independently inspectable or hot-swappable.",
  panels: new Map(),
  relationships: [
    {
      type: "dependency",
