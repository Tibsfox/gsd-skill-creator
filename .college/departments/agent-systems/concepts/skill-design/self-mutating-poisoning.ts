...
export const selfMutatingPoisoning: RosettaConcept = {
  ...
  description:
    "Fixed-payload skill poisoning ships one malicious instruction that a single-session review can catch. Self-mutating poisoning, characterized by SkillHarm (arXiv 2606.02540), instead ships a package that executes benignly on first invocation and then silently rewrites its own payload across later sessions, so the harmful behavior never appears in the audited state. Review certifies one point in a skill's lifecycle rather than its whole trajectory, meaning any task that later invokes the skill can be compromised. Ingesting third-party skills therefore demands re-verification at load time and cross-session change detection, not one-time vetting.",
  panels: new Map(),
  relationships: [
    {
      type: "dependency",
