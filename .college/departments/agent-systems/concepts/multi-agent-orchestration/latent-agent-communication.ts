...
export const latentAgentCommunication: RosettaConcept = {
  ...
  description:
    "Agents can coordinate over two media: discrete natural-language tokens, or the continuous representations under them — embeddings, hidden states, or shared KV-caches passed directly between models. Trading latent vectors instead of re-verbalizing skips the discretization bottleneck (sampling discards the full posterior) and avoids redundant re-encoding, lowering inference cost (arXiv 2606.05711v2, 2026). The price is interpretability: a hidden-state channel is not human-readable, so logging, guardrails, and heterogeneous-model transfer weaken or break. That 2606.05711v2 survey organizes 18 methods (2024–2026) along three axes — WHAT is passed (embeddings, hidden states, or KV-caches), WHICH sender–receiver alignment is used (latent-space and layer alignment), and HOW it is fused into the receiver (concatenation, prepending, mathematical operations, cross-attention, or cache restoration) — distilling five recurring design patterns, and it flags the security of an unreadable latent channel as an open problem. This names an axis the coordination surface leaves implicit — the communication medium — orthogonal to topology and dispatch policy. The 2026 lesson: choose the medium per edge — latent between trusted homogeneous models trading high-bandwidth state, natural-language wherever a message must be audited or read by a different agent.",
  panels: new Map(),
  relationships: [
    {
