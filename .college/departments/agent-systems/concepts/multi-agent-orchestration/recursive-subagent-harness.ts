...
export const recursiveSubagentHarness: RosettaConcept = {
  ...
  description:
    "The Recursive Agent Harness (RAH) makes the recursive unit a full agent harness — filesystem tools, code execution, and planning — rather than a bare model call, extending the model recursion of recursive language models into a code-first harness recursion (arXiv 2606.13643, 2026). A parent agent generates and runs an executable script that spawns subagent harnesses in parallel for fine-grained workloads while reserving structured function calls for small subtasks, decomposing long-context reasoning across independently-tooled children. Its distinct contribution is isolating the harness itself, not the model, as the source of the gain: holding the backbone fixed at GPT-5, RAH lifts the Codex baseline from 71.75% to 81.36% on Oolong-Synthetic, and reaches 89.77% with Claude Sonnet 4.5. The implication for agent systems is that orchestration structure drives long-context performance, so spawned subagents should carry full tooling rather than merely receive prompts.",
  panels: new Map(),
  relationships: [
    {
      type: "dependency",
