...
export const definiteDescriptions: RosettaConcept = {
  ...
  domain: 'logic',
  description:
    "Definite descriptions are phrases of the form 'the F' -- 'the author of Waverley', 'the present King of France'. Treating them as singular terms breaks classical logic: 'The present King of France is bald' has no truth value if the name denotes nothing. Russell's theory of definite descriptions (1905) instead eliminates 'the F' by contextual definition: 'The F is G' abbreviates the three-conjunct formula ∃x(Fx ∧ ∀y(Fy→y=x) ∧ Gx) -- existence, uniqueness, and predication. 'The F' is an incomplete symbol, meaningful only inside a whole sentence and carrying no denotation of its own. An empty description makes the sentence plainly false, preserving bivalence with no third truth value. Because the description unpacks into quantifiers, its scope matters: negating 'The F is G' is ambiguous between the narrow ¬∃x(…∧Gx) and the wide ∃x(…∧¬Gx). Whitehead and Russell mark this with the iota operator ℩x.Fx and explicit scope brackets. A modern proof-theoretic treatment recasts the contextual definition as a cut-free sequent calculus (Indrzejczak and Kürbis, arXiv:2606.11750v1, 2026): scope is fixed by the sequent rules that introduce a description — a lambda operator acting as an explicit scope marker to separate internal from external negation — rather than by hand-managed iota-bracket bookkeeping, and every such system satisfies cut elimination.",
  panels: new Map([
    ['python', {
      panelId: 'python',
