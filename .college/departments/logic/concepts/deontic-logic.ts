...
export const deonticLogic: RosettaConcept = {
  ...
  domain: 'logic',
  description:
    "Legal and moral reasoning is about what agents *ought* to do, not what merely *is* — yet classical logic only has truth. Deontic logic repairs this by adding a modal operator O ('it is obligatory that'), with permission Pphi = not O(not phi) as its dual and prohibition Fphi = O(not phi) as obligation-of-the-negation. Standard Deontic Logic (SDL) is the normal modal system KD: O is read over Kripke frames whose accessible worlds are the deontically ideal ones, so Ophi means phi holds in every acceptable world while phi may still be false in the actual world — a violated duty. Axiom D (seriality) forbids Ophi and O(not phi) together, which makes SDL explode on genuine moral dilemmas via obligation-aggregation. Conflict-tolerant (paraconsistent, non-adjunctive) deontic logics drop aggregation so clashing duties coexist without deriving triviality. It matters because legal norms, contracts, and AI-agent policies are exactly obligations, permissions, and prohibitions (arXiv:2606.30297v1, 2026).",
  panels: new Map([
    ['python', {
      panelId: 'python',
