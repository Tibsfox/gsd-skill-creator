...
export const intervalAlgebraTemporalRetrieval: RosettaConcept = {
  ...
  domain: 'logic',
  description:
    "Interval-Algebra Temporal Retrieval (IA-RAG) is a hierarchical temporal retrieval-augmented generation framework that treats knowledge not as static text but as time intervals, so retrieval can honor duration, overlap, and containment rather than coarse timestamps (arXiv 2606.06044, 2026). Facts become Interval Event Units organized into a hierarchical Thematic Forest whose temporal dependencies are governed by Allen's Interval Algebra, the calculus of the thirteen basic relations between two intervals. Its distinct contribution is a Sub-graph Time Tightening mechanism that refines fuzzy or incomplete interval boundaries by propagating logical constraints across connected event subgraphs, plus interval-algebra-guided traversal for implicit temporal semantics. For agent systems, this suggests time-stamped memory should store validity intervals and reason over them formally, letting an agent answer compositional temporal questions instead of matching literal dates.",
  panels: new Map([
    ['python', {
      panelId: 'python',
