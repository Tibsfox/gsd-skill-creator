/**
 * logic Department — June-2026 additional-scan batch-B concept tests.
 */
import { describe, it, expect } from 'vitest';
import * as barrel from './index.js';
import { defeasibleDeonticCompilation } from './index.js';
import type { RosettaConcept } from '../../../rosetta-core/types.js';

const concepts: RosettaConcept[] = [defeasibleDeonticCompilation];
const names = ["defeasibleDeonticCompilation"];
const localIds = new Set(Object.values(barrel as Record<string, RosettaConcept>).map((c) => c.id));

describe('logic Department — additional-scan batch-B concepts', () => {
  ...
  it.each(concepts.map((c, i) => [names[i], c] as const))('%s dept-local targetIds resolve', (_n, c) => {
    for (const rel of c.relationships) {
      if (rel.targetId.startsWith('logic-')) expect(localIds.has(rel.targetId)).toBe(true);
    }
  });
});
