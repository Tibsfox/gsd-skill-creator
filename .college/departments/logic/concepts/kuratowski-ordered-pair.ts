...
export const kuratowskiOrderedPair: RosettaConcept = {
  ...
  domain: 'logic',
  description:
    "Pure set theory has only one primitive relation, membership, and its sets are unordered: {a,b}={b,a}. Yet mathematics needs ordered pairs with (a,b)≠(b,a), so that relations and functions can be built. The Kuratowski ordered pair encodes order using sets alone: define (a,b) := {{a},{a,b}}. Its characteristic property — (a,b)=(c,d) iff a=c and b=d — is a theorem provable from extensionality by splitting on a=b versus a≠b; the first coordinate is recovered as the element common to both members, the second as the remaining element. Iterating, (a₁,…,aₙ):=((a₁,…,aₙ₋₁),aₙ) yields n-tuples, and the encoding embeds into the hierarchy of simple types. This matters because A×B ⊆ P(P(A∪B)), so Cartesian products, relations (sets of pairs), and functions (single-valued relations) all reduce to membership — set theory becomes a foundation for mathematics without new primitives (arXiv:2606.18474v1, 2026).",
  panels: new Map([
    ['python', {
      panelId: 'python',
