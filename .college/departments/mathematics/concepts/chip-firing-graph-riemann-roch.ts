...
export const chipFiringGraphRiemannRoch: RosettaConcept = {
  ...
  domain: 'mathematics',
  description:
    "A divisor on a finite connected graph G is an integer chip-count D over its vertices; firing a vertex sends one chip down each incident edge, changing D by a column of the graph Laplacian L = Deg − A. Two divisors are linearly equivalent when they differ by L·f for an integer firing script f, so Pic(G) = Z^V / im L, whose degree-0 part is the sandpile (critical) group. Define the rank r(D) combinatorially: r(D) = −1 if no equivalent divisor is effective, else the largest k such that D minus any k chips stays equivalent to an effective divisor. With canonical divisor K(v) = deg(v) − 2 and genus g = |E| − |V| + 1, the Baker–Norine theorem states r(D) − r(K − D) = deg(D) − g + 1, a perfect mirror of Riemann–Roch for algebraic curves. Effectivity and rank become computable through q-reduced divisors — the unique class representative reachable by Dhar's burning algorithm. The 2026 contribution is a complete formal proof of this graph Riemann–Roch theorem in the Lean 4 proof assistant: it mechanizes the existence and uniqueness of q-reduced divisors, a modified Dhar's burning algorithm, the bijection between acyclic orientations with a unique source and maximal superstable configurations, and Clifford's theorem, while flagging further extensions as open challenges for future formalization. (arXiv:2606.16679v1, 2026)",
  panels: new Map([
    ['python', {
      panelId: 'python',
