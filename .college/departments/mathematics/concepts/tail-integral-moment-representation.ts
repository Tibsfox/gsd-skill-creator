...
export const tailIntegralMomentRepresentation: RosettaConcept = {
  ...
  domain: 'mathematics',
  description:
    "How large must a moment's order grow before it stops existing? For a random variable X with cumulative distribution F and survival function S(t)=P(X>t), the tail-integral (layer-cake) moment representation writes every real-order moment as a weighted integral of these tail functions: for p>0, E[X^p]=∫₀^∞ p·t^(p-1)·S(t) dt, while negative moments E[X^(-q)]=q·∫₀^∞ t^(-q-1)·F(t) dt are governed instead by the CDF near the origin. Writing X^p=∫₀^X p·t^(p-1) dt and applying Tonelli turns an abstract expectation into a statement about how fast S decays: exponentially light tails make every positive moment finite, whereas a power-law tail S(t)∼t^(-α) admits E[X^p] only for p<α. Letting p→0 recovers the logarithmic moment E[log X]=∫₀^∞ (e^(-t)−L_X(t))/t dt, a Frullani integral tying log-moments to the Laplace transform L_X. The construction unifies positive, fractional, and negative real-order moments under one transform, with the classical Darth-Vader identity E[X]=∫₀^∞ S(t) dt for non-negative X appearing as the p=1 special case. For discrete laws the same tail construction yields explicit series representations in terms of the cumulative probabilities, giving simple criteria for which moments exist — worked out for the zeta and Skellam distributions (arXiv:2606.14019v1, 2026).",
  panels: new Map([
    ['python', {
      panelId: 'python',
