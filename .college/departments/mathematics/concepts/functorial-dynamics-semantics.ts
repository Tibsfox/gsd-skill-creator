...
export const functorialDynamicsSemantics: RosettaConcept = {
  ...
  domain: 'mathematics',
  description:
    "Classical mechanics, gradient-based learning, and lattice diffusion look like unrelated dynamical theories, yet each is an open system that composes: wire outputs to inputs and the whole evolves as the assembled parts dictate. The construction, functorial dynamics semantics, fixes an operad O of smooth adaptive arrangements — boxes with typed ports and smooth update laws — and interprets it by a functor into Poly, the category of polynomial coalgebras, where a system is a state set S with a lens S → p(S) that reads inputs and emits outputs. Because the interpretation is functorial, every wiring diagram in O maps to a composite of lenses, so a network's dynamics is fixed by its components. The same operad admits two functors: a variational reading giving Hamilton's equations and a descent reading giving gradient flow. On a graph with Laplacian L, the single potential ½⟨u,Lu⟩ yields the heat equation u̇ = -Lu and, at second order, the discrete wave equation ü = -Lu. Compositionality, not coincidence, unifies them. (arXiv:2606.28984v1, 2026)",
  panels: new Map([
    ['python', {
      panelId: 'python',
