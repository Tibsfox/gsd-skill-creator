...
export const geometricGraphManifoldRecovery: RosettaConcept = {
  ...
  domain: 'mathematics',
  description:
    "Sample n points i.i.d. from a smooth density on a compact d-dimensional Riemannian manifold M and join any two within a radius r_n: this is the random geometric graph (RGG) on M. Equip its giant component with the graph (hop-count) distance rescaled by r_n. The theorem: once the expected average degree Δ exceeds a critical threshold Δ_c that depends only on the dimension d, so near-geodesic paths percolate, this rescaled graph metric converges to (M, geodesic distance) in Gromov-Hausdorff distance — the infimum, over correspondences between the two spaces, of the worst pairwise distance distortion. Intuitively each hop advances about r_n along M, so r_n times the hop count tracks geodesic length; below the threshold, forced detours inflate it and recovery fails. This makes precise why shortest-path methods like Isomap reconstruct a manifold's entire geometry — not merely pointwise distances — from a point cloud. (arXiv:2606.01627v1, 2026)",
  panels: new Map([
    ['python', {
      panelId: 'python',
