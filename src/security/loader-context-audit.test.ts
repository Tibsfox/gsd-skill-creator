...
const KNOWN_UNWIRED: ReadonlySet<string> = new Set([
  ...
  // v1.49.909: src/intelligence/kb/store.ts CHIPPED VIA VERDICT (NOT a wire). KBStore is SQLite-backed via better-sqlite3 native binding; the only node:fs usage is mkdirSync (write-side, out-of-scope per #10457). Added "Role: NOT a disk loader" header per the audit-test convention — file no longer needs KNOWN_UNWIRED entry. First instance of "Role: NOT a disk loader" verdict applied as the closing-move on a multi-chip campaign — distinct from a wire-up. KNOWN_UNWIRED Loader closes to 0 at the end of this ship. 1 → 0.
