...
describe('boundedLearningCommand — --summary mode (v1.49.801)', () => {
  ...
  it('emits JSON summary of all 8 wired thresholds with currentValue + observationSource', async () => {
    const code = await boundedLearningCommand(
      ['--config', configPath, '--audit-log', auditLogPath, '--summary'],
    );
    expect(code).toBe(0);
    const out = JSON.parse(collectLog());
    expect(out.thresholds).toHaveLength(8);
    expect(out.wiredThresholdCount).toBe(8);
