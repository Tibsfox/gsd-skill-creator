...
Scanned all 28 markdown files in `www/PNW/VAV/research/` for module cross-references using the pattern `M{N}`. Classified each reference as:
...
Three experiments stored in `www/PNW/VAV/poc/`. All are self-contained Python scripts with no external dependencies.
