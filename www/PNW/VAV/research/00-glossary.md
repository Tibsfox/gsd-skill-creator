...
| **TSR** | Temporal Super-Resolution. A technique using multi-channel active illumination synchronized to a camera to extend effective temporal range beyond the hardware Nyquist limit by a factor of up to 6. Ben-Ezra et al. (2024). In VAV, parallels palette super-resolution via multi-section encoding. | M14 |
| **CUP** | Compressed Ultrafast Photography. Achieves trillions of frames per second through spatiotemporal multiplexing on a single sensor exposure — trading spatial resolution for temporal density via compressed sensing reconstruction. | M14 |
| **MTF** | Modulation Transfer Function. Measures how faithfully an imaging system reproduces spatial or temporal frequency content. A perfect system has MTF = 1 at all frequencies. The camera shutter shapes temporal MTF; the Tessive Time Filter optimizes it. | M14 |
| **Deinterlacing** | Reconstructing progressive frames from interlaced fields. Methods ranked by quality: bob (interpolate), weave (combine), motion-adaptive (per-pixel), motion-compensated (optical flow). In VAV, parallels RADOS erasure coding reconstruction strategies. | M14 |
| **CRI** | Color Rendering Index. Measures a light source's ability to faithfully render 8 (Ra) or 14 (Re) test color samples relative to a reference illuminant. Studio lighting for color fidelity requires CRI > 95. | M15 |
| **Bayer Pattern** | The color filter array (RGGB) used in most digital camera sensors. Each photosite captures only one color channel; demosaicing interpolates the other two. This is a spatial subsampling decision — trading color resolution for luminance resolution. | M15 |
| **RDMA** | Remote Direct Memory Access. A technology allowing one computer to read/write another's memory without involving the remote OS, eliminating kernel crossings and memory copies. Used in InfiniBand for Ceph OSD-to-OSD communication via the Verbs API. | M17 |
| **SHARP** | Scalable Hierarchical Aggregation and Reduction Protocol. NVIDIA technology that performs MPI allreduce operations inside the InfiniBand switch ASIC rather than on CPU nodes, dramatically reducing latency for distributed computing. | M17 |
| **HIPPI** | High-Performance Parallel Interface (ANSI X3.183, 1990). 800 Mbps simplex copper interconnect used in Cray/SGI supercomputers. First high-speed fabric to use credit-based flow control — an idea that influenced InfiniBand design. | M17 |
| **Reed-Solomon** | A family of error-correcting codes that can correct up to t symbol errors in a codeword. Used in CDs, DVDs, QR codes, DSL, and DVB-S2. In Ceph, erasure coding pools use Reed-Solomon to distribute data and parity across OSDs. | M17, M18 |
| **DOCSIS** | Data Over Cable Service Interface Specification. Uses cable TV coaxial plant's 5-1000 MHz spectrum. DOCSIS 3.1 (2013) uses OFDM with 4K-point FFT and QAM-4096. Shared-medium: users in a neighborhood share downstream spectrum. | M18 |
| **DTN** | Delay-Tolerant Networking. A network architecture for environments with intermittent connectivity, using store-and-forward bundles (RFC 5050). Applied in sneakernet mesh, interplanetary Internet (IPN), and disaster recovery scenarios. | M18 |
| **Bundle Protocol** | RFC 5050. The transport protocol for delay-tolerant networks. Nodes carry bundles along physical routes with custody transfer (signed acknowledgment at each hop). Used by Interplanetary Internet (Cerf, Burleigh et al.) for spacecraft communications. | M18 |
| **Arrow Flight** | A gRPC-based protocol for streaming Apache Arrow columnar record batches between processes. Designed for high-throughput data federation — the transport layer connecting heterogeneous data stores in a sovereign world federation. | M19 |
| **3-2-1-1-0** | Extended backup rule: 3 copies of data, on 2 different media types, with 1 copy offsite, 1 copy offline/air-gapped, and 0 errors verified on all copies. Applied to VAV as: primary Ceph + RBD mirror + BorgBackup + LTO tape + checksum verification. | M19 |
| **BorgBackup** | A deduplicating archiver with compression and authenticated encryption. Block-level dedup achieves excellent storage efficiency for Minecraft worlds where most chunks are unchanged between saves. Used as the cold backup tier in VAV. | M19 |
| **CAEP** | Continuous Access Evaluation Profile. A protocol that propagates security events (session revocation, credential change, device posture change) to resource servers in real time. The gap identified in VAV's zero trust architecture — needed for Minecraft player session validation. | M20 |
| **NGFW** | Next-Generation Firewall. Combines stateful packet inspection with application awareness, IPS, user identity, and SSL inspection in one appliance. Vendors: Palo Alto, Fortinet, Check Point. Represents the application-layer security tier in VAV zone architecture. | M20 |
