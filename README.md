...
An adaptive learning and coprocessor architecture for [Claude Code](https://docs.anthropic.com/en/docs/build-with-claude/claude-code), built as an extension to [GSD (Get Shit Done)](https://github.com/gsd-build) -> now [GSD(open-gsd)](https://github.com/open-gsd/get-shit-done-redux)
...
| Channel | Type | Use |
|---------|------|-----|
| **Mail** | Async durable | Task assignments, status reports, results that persist |
...
Gastown is not a runtime dependency. Its coordination patterns are encoded as a declarative chipset (skills, types, and configuration) that teaches Claude Code how to orchestrate multiple agents. See the [Gastown Integration Guide](docs/gastown-integration/) for the full 10-document reference.
